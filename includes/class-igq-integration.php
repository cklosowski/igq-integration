...
class Affiliate_WP_IGQ extends Affiliate_WP_Base {
	...
	*/
	 ...
	 * @since   1.2
	 */
	public $context = 'igq-integration';
	...
	*/
	public function init() {
		...
		add_filter( 'woocommerce_product_data_tabs', array( $this, 'add_product_tab' ), 10, 1 );
		add_action( 'woocommerce_product_data_panels', array( $this, 'render_product_tab' ), 10 );
		add_action( 'woocommerce_process_product_meta', array( $this, 'save_product_tab_details' ), 10, 1 );
	...
	public function add_pending_referral( $order_id = 0 ) {
		...
		foreach ( $items as $product ) {
			...
			$affiliate_id = get_post_meta( $product['product_id'], 'igq_assigned_affiliate', true );
			if ( $affiliate_id < 1 ) {
				$this->log( 'Product did not have an assigned affiliate' );
				return;
			}
...
<?php
	...
	public function add_product_tab( $tabs ) {
		$tabs['product_affiliate_relationship'] = array(
			'label'    => 'Affiliate Ownership',
			'priority' => 50,
			'target'   => 'affiliate_ownership',
		);
		return $tabs;
	}
	...
	public function render_product_tab() {
		...
		?>
		<div id="affiliate_ownership" class="fee_panel panel woocommerce_options_panel wc-metaboxes-wrapper">
			<div class="options_group">
				...
				$affiliate_options = array( '-1' => 'Select an Affiliate' );
				foreach ( $affiliates as $affiliate ) {
					$user = new WP_User( $affiliate->user_id );
					$affiliate_options[ $affiliate->affiliate_id ] = affwp_get_affiliate_username( $affiliate->affiliate_id );
				}

				woocommerce_wp_select(
					array(
						'label'   => 'Assigned Affiliate',
						'id'      => 'igq_assigned_affiliate',
						'dsc_tip' => 'The affiliate you would like associated with this product',
	...
	public function save_product_tab_details( $post_id ) {
		if ( '-1' === $_POST['igq_assigned_affiliate'] ) {
			delete_post_meta( $post_id, 'igq_assigned_affiliate' );
		} else {
